...
// Function to get the AI client, ensuring API_KEY is present.
const getAiClient = () => {
    const apiKey = process.env.API_KEY;
    if (!apiKey) {
        // This error will now be caught by functions calling it,
        // but the server startup check in index.ts should prevent this from being hit.
...
**Catálogo de Produtos (Apenas os produtos ATIVOS estão listados):**
${productCatalog || 'Nenhum produto cadastrado e ativo no momento.'}
...
export const generateChatResponse = async (
    ...
    try {
        ...
        if (!lastMessage || lastMessage.role !== 'user') {
             // If last message is not from user, it means AI just sent a message. Don't respond.
            return "";
        }
